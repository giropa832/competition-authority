 ...
 */
export function interpolatePM(percentile) {
    if (percentile < 30)
        return null;
 ...
 */
export function pmToMedalLevel(pm) {
    if (pm >= 9.1)
        return "gran_prestige_gold";
    if (pm >= 7.75)
        return "gold_especial";
    if (pm >= 6.0)
        return "gold";
    if (pm >= 4.0)
        return "silver";
 ...
 */
export function classifyTier(acScore, category) {
    if (category === "international") {
        if (acScore >= 80)
            return "tier_1";
        if (acScore >= 60)
 ...
 */
export function calculateFIM(goldRate) {
    if (goldRate <= 0.3)
        return 1.0;
 ...
 */
export function calculateFH(evooCountry, competitionCountry) {
 ...
 */
export function scoreToPercentile(score, distribution) {
 ...
 */
export function resolvePM(result, distribution) {
 ...
 */
export function calculateEvooScore(evoo, authorities, competitions, distributions, // key: "competitionId:season"
season // optional: filter results to a single season
) {
    let mainScore = 0;
    let scoreNE = 0;
    let scoreAuxiliar = 0;
    ...
    const distFallbackMap = new Map();
    const distFallbackSeasons = new Map();
 ...
 */
export function buildRanking(evoos, authorities, competitions, distributions, season) {
    const scored = evoos
        .map((evoo) => {
        const { score, scoreNE, scoreAuxiliar, medalCount, breakdown } = calculateEvooScore(evoo, authorities, competitions, distributions, season);
        return {
            ...evoo,
            score,
            scoreNE,
 ...
 */
export function scoreAllEvoos(evoos, authorities, competitions, distributions, season) {
 ...
 */
export function buildCountryRanking(allScoredEvoos, countryCode) {
 ...
 */
export function buildVarietyRanking(allScoredEvoos, varietySlug, countryCode) {
 ...
 */
export function buildCategoryRanking(allScoredEvoos, categorySlug, hasTag, countryCode) {
    let categoryEvoos = allScoredEvoos.filter((a) => hasTag(a, categorySlug));
    if (countryCode) {
        categoryEvoos = categoryEvoos.filter((a) => a.producer.country === countryCode);
    }
